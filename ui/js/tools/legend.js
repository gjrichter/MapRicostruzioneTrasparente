	...
	ixmaps.legend.makeColorLegendHTMLCompact = function(szId,szLegendId){
		...
		for ( var row=0; row<nRows; row++){
			...
			for ( var i=0; i<colorA.length; i++){
				...
				var nCount = Math.min(100/colorA.length,8);
		...
		var span = Math.floor(colorA.length*0.5);
		szHtml += "<tr class='legend-range-text' >";
		szHtml += "<td colspan='"+(span)+"' >"+ixmaps.__formatValue(themeObj.nMin,2,"BLANK")+" "+themeObj.szUnits+"</td>";
		szHtml += "<td colspan='"+(colorA.length - span)+"' align='right'>"+ixmaps.__formatValue(themeObj.nMax,2,"BLANK")+" "+themeObj.szUnits+"</td>";
		szHtml += "</tr>";
